    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="flex flex-col justify-between">
          ...
          <div className="flex items-center gap-3 my-2">
            ...
            <span className="text-3xl font-extrabold text-slate-900 dark:text-slate-100">
              {analysis.riskNumerical} <span className="text-xs font-normal text-slate-400">/ 100 Risk Index</span>
            </span>
          </div>
        ...
        <Card className="flex flex-col justify-between">
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-wider">
            <Users className="w-4 h-4 text-blue-500" /> Contracting Parties
          </div>
          <div className="space-y-1.5 my-2">
            {analysis.parties.map((p, idx) => (
              <div key={idx} className="flex items-center justify-between text-xs font-semibold text-slate-800 dark:text-slate-200">
                <span>{p.name}</span>
                <span className="text-[10px] text-blue-600 bg-blue-50 dark:bg-blue-950 px-2 py-0.5 rounded-full">{p.role}</span>
              </div>
            ))}
          </div>
          <span className="text-[11px] text-slate-400">Verified identity & Delaware governing jurisdiction</span>
        </Card>

        {/* Important Dates */}
        <Card className="flex flex-col justify-between">
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-wider">
            <Calendar className="w-4 h-4 text-indigo-500" /> Critical Dates & Milestones
          </div>
          <div className="space-y-1.5 my-2">
          ...
          <span className="text-[11px] text-rose-500 font-semibold">90-day non-renewal notice deadline approaching</span>
        </Card>
      </div>
        ...
        <div className="space-y-4">
          ...
          <Card className="space-y-3">
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-blue-600" /> Executive AI Summary
            </h3>
              ...
              <ul className="space-y-2">
                {analysis.key_clauses.filter(c => c.riskLevel === 'High').length > 0
                  ? analysis.key_clauses.filter(c => c.riskLevel === 'High').map(c => (
                      <li key={c.id} className="text-xs text-rose-800 dark:text-rose-300 space-y-0.5">
                  ...
                  : <li className="text-xs text-rose-500">No high risk clauses detected.</li>
                }
              </ul>
            </div>
              ...
              <ul className="space-y-2">
                {analysis.key_clauses.filter(c => c.riskLevel === 'Low').length > 0
                  ? analysis.key_clauses.filter(c => c.riskLevel === 'Low').map(c => (
                      <li key={c.id} className="text-xs text-emerald-800 dark:text-emerald-300 space-y-0.5">
                        <span className="font-bold block">{c.section}: {c.title}</span>
                        <span className="block">{c.summary}</span>
                      </li>
                    ))
                  : <li className="text-xs text-emerald-600">No protective provisions found.</li>
                }
              </ul>
            </div>
          </div>

          {/* Medium Risk Clauses - full width */}
          {analysis.key_clauses.filter(c => c.riskLevel === 'Medium').length > 0 && (
            <div className="p-4 rounded-2xl bg-orange-50/60 dark:bg-orange-950/20 border border-orange-200/60 dark:border-orange-900/40">
              <h4 className="text-xs font-bold text-orange-900 dark:text-orange-200 flex items-center gap-2 mb-2">
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {analysis.key_clauses.filter(c => c.riskLevel === 'Medium').map(c => (
                  <li key={c.id} className="text-xs text-orange-800 dark:text-orange-300 space-y-0.5">
                    <span className="font-bold block">{c.section}: {c.title}</span>
                    <span className="block">{c.summary}</span>
                    {c.recommendation && (
                      <span className="block italic text-orange-600 dark:text-orange-400">→ {c.recommendation}</span>
                    )}
                  </li>
                ))}
              </ul>
          ...
          {/* Missing Clauses - from backend JSON */}
              ...
              <ul className="space-y-3">
                {analysis.missing_clauses.map(m => (
                  <li key={m.id} className="space-y-1">
                    ...
                    <p className="text-xs text-amber-800 dark:text-amber-300 ml-0">{m.description}</p>
          ...
          {/* AI Recommendations - from backend JSON */}
          {analysis.recommendations.length > 0 && (
            <div className="p-4 rounded-2xl bg-blue-50/60 dark:bg-blue-950/20 border border-blue-200/60 dark:border-blue-900/40">
              <h4 className="text-xs font-bold text-blue-900 dark:text-blue-200 flex items-center gap-2 mb-3">
                <Zap className="w-4 h-4 text-blue-600" />
                AI Recommended Action Items ({analysis.recommendations.length})
              </h4>
              <ul className="space-y-3">
                {analysis.recommendations.map((rec, idx) => (
                  <li key={rec.id} className="space-y-1 border-b border-blue-200/40 dark:border-blue-800/40 last:border-0 pb-2 last:pb-0">
                    <div className="flex items-center gap-2">
                      <span className="w-5 h-5 rounded-full bg-blue-600 text-white text-[10px] font-bold flex items-center justify-center shrink-0">{idx + 1}</span>
                      <span className="text-[10px] font-bold uppercase text-blue-600 dark:text-blue-400">{rec.category}</span>
                      <span className="text-xs font-bold text-blue-900 dark:text-blue-100">{rec.title}</span>
                    </div>
                    <p className="text-xs text-blue-700 dark:text-blue-300 ml-7">{rec.description}</p>
                    <p className="text-xs font-semibold text-blue-800 dark:text-blue-200 ml-7">👉 {rec.actionItem}</p>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="space-y-4">
          <Accordion
            items={analysis.key_clauses.map((c) => ({
              ...
              title: (
                <div>
                  <span className="text-xs font-bold text-blue-600 dark:text-blue-400 block">{c.section}</span>
              ...
              content: (
                <div className="space-y-3">
                  <div className="p-3 rounded-xl bg-slate-100/80 dark:bg-slate-800/80 font-mono text-xs text-slate-800 dark:text-slate-200">
                    &quot;{c.content}&quot;
                  </div>
                  <p className="text-xs text-slate-600 dark:text-slate-300">
                    <strong className="text-slate-900 dark:text-slate-100">AI Risk Assessment:</strong> {c.summary}
                  </p>
                  {c.recommendation && (
                    <div className="p-3 rounded-xl bg-blue-50 dark:bg-blue-950/60 border border-blue-200 dark:border-blue-800 text-xs text-blue-800 dark:text-blue-200">
      ...
      {activeTab === 'recommendations' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {analysis.recommendations.map((rec) => (
            <Card key={rec.id} hoverLift className="space-y-2">
              <span className="text-[10px] font-extrabold uppercase tracking-wider text-blue-600">{rec.category}</span>
              ...
              <div className="pt-2">
                <span className="text-xs font-semibold text-emerald-600 block">Recommended Action:</span>
                <p className="text-xs text-slate-700 dark:text-slate-300 mt-0.5">{rec.actionItem}</p>
              </div>
            </Card>
          ))}
        </div>
      ...
      {activeTab === 'json' && (
        <Card className="space-y-4">
          ...
          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 font-mono text-xs text-emerald-400 overflow-x-auto max-h-[500px]">
